...
export const GraphModule = ({ onBackToDashboard }: { onBackToDashboard: () => void }) => {
    ...
    const runBFS = () => {
        ...
        // Mark A as failed
        setNodes(prev => prev.map(n => n.id === 'A' ? { ...n, status: 'failed' } : { ...n, status: 'idle' }));
        ...
        const traverse = () => {
            ...
            vis.add(current);
            setQueue([...q]);
            setExecutionLog(log => [...log, `📍 Processing: ${current}`].slice(-6));
    ...
    const runCrawler = () => {
        ...
        const crawl = () => {
            if (toVisit.length === 0) {
                setCrawlerNode(null);
                setExecutionLog(log => [...log, `✅ Crawler Complete - Total Network Calls: ${callCount}`].slice(-6));
            ...
            callCount++;
            setNetworkCalls(callCount);
            setExecutionLog(log => [...log, `🌐 Network Call #${callCount}: Fetch metadata for ${current}`].slice(-6));
